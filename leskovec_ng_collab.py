...
def get_ground_truth():
    ...
    true_labels = loadmat('Datasets/LN_true.mat')
    true_labels = true_labels['s_LNG'] - 1 ##making label start from 0 true labels[1,2]
...
def get_graph_matrix():
        ...
        # load data
    L1 = loadmat('Datasets/LN_1995_1999.mat')
    L2 = loadmat('Datasets/LN_2000_2004.mat')
    L3 = loadmat('Datasets/LN_2005_2009.mat')
    L4 = loadmat('Datasets/LN_2010_2014.mat')

    G1 = nx.from_numpy_matrix(L1["A1"],create_using=nx.MultiGraph())
    G2 = nx.from_numpy_matrix(L2["A2"],create_using=nx.MultiGraph())
    G3 = nx.from_numpy_matrix(L3["A3"],create_using=nx.MultiGraph())
    G4 = nx.from_numpy_matrix(L4["A4"],create_using=nx.MultiGraph())

    all_graphs = [G1,G2,G3,G4]
    F = nx.compose_all(all_graphs)
    X = nx.to_scipy_sparse_matrix(F)
...
if __name__ == '__main__':
    ...
    for train_split, test_split in kfold.split(IDX):
        start = time.time()
        print "Folds:",cnt
        AUC_test[cnt] = innerfold(Ground_Truth_labels, train_split, test_split)
